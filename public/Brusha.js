...
class Brusha {
  constructor(ele) {
    ...
    this.get = {
      ...
      color: () => {
        ...
        return this.colorGradientEle.getAttribute('color-values').split(',');
      },
    ...
    this.colorInputHandle = {
      updateGradient: () => {
        const colorValues = [...this.colorGradientEle.querySelectorAll('input[type=color]')].map((_) => _.value).join(',');
  ...
  circleMove = (e) => {
    ...
    const calcDist = ({ x, y }, { toX, toY }) => Math.sqrt((toX - x) ** 2 + (toY - y) ** 2);
  ...
  init = () => {
    ...
    this.colorEle.querySelector('.add-point').onclick = () => {
      ...
      const inps = [...this.colorGradientEle.querySelectorAll('input[type=color]')];
